...
//   const [originalAgentData, setOriginalAgentData] = useState<Agent | null>(null);
...
import {
  ...
  getAgentById,
  updateAgentInfo,
} from "../../utils/api"; // Assuming updateAgent and getAgentById exist or will be created
...
export default function EditAgentForm() {
  ...
  const [isDistrictDropdownOpen, setIsDistrictDropdownOpen] =
    useState<boolean>(false);
  const [agentStatus, setAgentStatus] = useState<string>(""); // New state for agent status
  const [isStatusDropdownOpen, setIsStatusDropdownOpen] =
    useState<boolean>(false); // New state for status dropdown
  ...
  useEffect(() => {
    const fetchAgentData = async () => {
      ...
      setError("");
      try {
        const response = await getAgentById(token, id); // Assuming getAgentById exists
        if (response.success && response.data) {
          ...
          setOriginalAgentData(agent); // Store original agent data
        } else {
