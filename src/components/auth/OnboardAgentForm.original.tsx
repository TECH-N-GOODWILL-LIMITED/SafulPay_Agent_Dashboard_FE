  ...
  const handlePhoneBlur = async (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (formattedPhoneNumber.length !== 11) {
        setWarnError(true);
        setUserExistError("Invalid phone number format.");
        return;
      }
      ...
      if (response.data?.status) {
        ...
        setUserExistError("User not found. Advice to register on SafulPay app");
      }
    } catch (err) {
      setUserExistError(`Error checking user existence - ${!err}`);
    }
  };
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input
                  ...
                  error={warnError || (!!error && !phone) || !!userExistError}
                  selectedCountries={["SL"]}
                  hint={userExistError || ""}
                />
              </div>
